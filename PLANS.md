...
- [x] (2026-05-10 16:02 CST) Fixed the user-visible hosted-analysis error card for low-level transport failures by normalizing `IOException` messages such as `unexpected end of stream on com.android.okhttp.Address...` into a localized network/proxy retry message in `RemodelViewModel`, applying the same mapping to plan generation and preview network errors, and adding a regression test so raw OkHttp internals no longer appear in the upload flow UI.
- [x] (2026-05-09 11:29 CST) Adjusted the refreshed `Second_Bloom用户手册_V1.1.docx` screenshots after the user manually replaced them: normalized all 12 embedded phone screenshots to a consistent 7.0 cm display width, centered the image paragraphs, added the missing `图 3-11  登录与注册页面` caption for the newly added login/register screenshot, regenerated `output/doc/Second_Bloom用户手册_V1.1_20260424174913.pdf`, and verified the PDF renders on A4 with 27 pages and includes the new caption text.
- [x] (2026-05-08 23:54 CST) Refreshed all 11 software-copyright user-manual screenshots from the current `emulator-5554` debug build, covering the Chinese inspiration, upload, low-confidence, analysis summary, plan generation, preview editor, preview result, wardrobe, planet, profile, and Clerk login screens; replaced the matching assets under `output/doc/assets/soft-copyright`, embedded those screenshots into `output/doc/Second_Bloom用户手册_V1.1.docx`, regenerated `output/doc/Second_Bloom用户手册_V1.1_20260424174913.pdf`, and verified the DOCX media hashes match the refreshed assets and the PDF renders as A4 with 25 pages.
- [x] (2026-05-08 18:03 CST) Diagnosed the screenshot's `识别失败 / connection closed` as an emulator network/DNS issue rather than an app or backend failure: direct host multipart upload to `POST /analyze-garment` returned `200`, Vercel logs showed no emulator request before the fix, emulator DNS resolved Vercel and Google domains to non-Vercel/non-Google IPs, and setting the emulator proxy to the host proxy with `adb -s emulator-5554 shell settings put global http_proxy 10.0.2.2:7897` restored network validation and allowed the same `55.jpg` upload to return `识别结果已准备好`.
- [x] (2026-05-08 18:00 CST) Restored hosted backend reachability for debug builds by disabling Vercel SSO deployment protection on the sibling `second-bloom-backend` project, verifying `https://second-bloom-backend-juliantiens-projects.vercel.app/health/live` and `/health/ready` return `200 {"status":"ok"}` through ordinary `curl`, and repointing local debug configuration away from the DNS-broken `https://second-bloom-backend.vercel.app` alias to the reachable team alias.
- [x] (2026-05-08 17:49 CST) Implemented the full-page system-bar overlap fix by adding shared Compose inset modifiers, applying top-level status-bar padding to the four bottom-tab pages, applying system-bar plus IME padding to the remodel/detail/preview flow pages and account wrapper, and moving the custom bottom navigation above the system navigation/gesture area.
- [x] (2026-05-08 17:49 CST) Verified the inset fix with `sh gradlew assembleDebug --console=plain`, `sh gradlew installDebug --console=plain`, `sh gradlew testDebugUnitTest --console=plain`, and `sh gradlew assembleDebugAndroidTest --console=plain`; adb/UI-tree smoke checks on `emulator-5554` confirmed `灵感空间`, `数字衣橱`, `可持续星球`, `我的主页`, `上传旧衣`, `灵感详情`, and the login wrapper start below the status bar and keep the bottom bar above the gesture area.
- [x] (2026-04-26 16:08 CST) Fixed the post-login crash reproduced after Clerk sign-in by isolating cloud-history sync exceptions inside `RemodelViewModel.startActiveHistorySyncJob()`, adding a regression test for `/me/history/bootstrap` failure, re-verifying `sh gradlew testDebugUnitTest --console=plain`, and reinstalling the patched debug build on `emulator-5554` with `sh gradlew installDebug --console=plain`.
- [x] (2026-04-26 15:34 CST) Implemented the diagnostic follow-up plan for the Android app: fixed the Compose lint blocker by collecting `RemodelViewModel.uiState` with `collectAsState()` inside `RemodelScreensTest`, stabilized the `PreviewEditorScreen` instrumentation test with explicit Compose test tags, split the Clerk deep-link manifest data tags, made low-risk locale/modifier lint cleanups, and verified `sh gradlew testDebugUnitTest --console=plain`, `sh gradlew lintDebug --console=plain`, `sh gradlew assembleDebugAndroidTest --console=plain`, and `sh gradlew connectedDebugAndroidTest --console=plain` all complete successfully on `emulator-5554`.
- [x] (2026-04-26 15:34 CST) Re-ran Android emulator smoke checks with the debug build still pointed at `https://second-bloom-backend.vercel.app`: the demo asset flow reaches `识别结果已准备好` without any `No content provider: demo://...` logcat hit, and a system Photo Picker-selected image reaches the real backend path and returns a low-confidence recognition result instead of a content-provider failure.
- [x] (2026-04-26 15:34 CST) Re-ran the sibling backend's non-Docker verification: `.venv/bin/python -m compileall -q app api workers`, `.venv/bin/python -m pytest -q -rs`, and `.venv/bin/python -m pip wheel . --no-deps -w /tmp/second-bloom-backend-wheel` pass. Full local integration verification remains blocked on this machine because `docker`, `colima`, and `podman` are not available on `PATH`, so the Postgres/Redis/MinIO compose stack cannot be started here.
- [x] (2026-04-26 15:29 CST) Verified the inspiration header fix on `emulator-5554`: `sh gradlew assembleDebug --console=plain` and `sh gradlew installDebug --console=plain` completed successfully, the launched app screenshot at `/tmp/secondbloom-inspiration-statusbar-fix.png` shows the status bar separated from the title, and the UI tree places the inspiration feed root at `y=159` with the `灵感空间` title starting at `y=207`.
- [x] (2026-04-26 15:28 CST) Fixed the inspiration feed's edge-to-edge header overlap by applying status-bar padding to the root `LazyVerticalStaggeredGrid` in `app/src/main/java/com/scf/secondbloom/ui/screens/InspirationScreen.kt`, keeping the existing bottom navigation inset behavior unchanged.
- [x] (2026-04-26 14:33 CST) Ran a current project diagnostic pass across the Android app and sibling backend: `sh gradlew lintDebug --console=plain` fails with 3 Compose lint errors in `RemodelScreensTest.kt`, `sh gradlew testDebugUnitTest --console=plain` passes, `sh gradlew assembleDebugAndroidTest --console=plain` passes, `sh gradlew connectedDebugAndroidTest --console=plain` now runs 18 instrumentation tests on `emulator-5554` with 1 product-level failure, and the sibling backend passes `.venv/bin/python -m compileall -q app api workers`, `.venv/bin/python -m pytest -q`, and `.venv/bin/python -m pip wheel . --no-deps`.
- [x] (2026-04-24 17:46 CST) Updated `output/doc/新软件信息采集表-Second Bloom版.docx` after the user supplied the refreshed `Second_Bloom用户手册_V1.1.docx`: set the information sheet's `手册多少页（必填）` value to the user-confirmed `28页`, regenerated `output/doc/新软件信息采集表-Second Bloom版.pdf`, and verified the rendered PDF text shows `手册多少页（必填） 28页` while keeping the code page count at `60页`.
- [x] (2026-04-24 17:05 CST) Fixed the Chinese plan-generation regression by preserving `response_language` inside the sibling backend's `GenerateRemodelPlansService`, forwarding `responseLanguage` through the backend real-provider adapter, adding Android and backend regression tests for the language field, documenting the response-language contract for `POST /generate-remodel-plans`, deploying the backend to Vercel production deployment `dpl_kFo5fPZoENbrJgYHG8ooL6Fp6VBT`, and smoke-testing that the production alias returns Chinese plan titles, summaries, materials, and steps for `responseLanguage="zh"`.
- [x] (2026-04-24 16:05 CST) Updated the software-copyright document generator and regenerated all soft-copyright DOCX/PDF outputs with the supplied applicant facts: copyright owner `张子妍`, company founding date `2025年12月01日`, software completion date `2026年03月05日`, and first publication date `2026年04月16日`. The certificate/ID field remains intentionally marked as pending because the number has not been provided.
- [x] (2026-04-24 15:55 CST) Re-reviewed the software-copyright deliverables against the NyxGuard reference materials, fixed `新软件信息采集表-Second Bloom版.docx` to copy the completed NyxGuard template and replace only the content cells, preserving the original 27-row, 3-column table structure, margins, row labels, rights-holder section, and applicant-information row.
- [x] (2026-04-24 15:55 CST) Re-captured the software-copyright handbook screenshots from the Android emulator after switching the app UI to Chinese, replacing the remaining English flow screenshots (`03` through `07`) and the incorrect wardrobe/planet captures (`08` and `09`) under `output/doc/assets/soft-copyright`.
- [x] (2026-04-24 15:55 CST) Re-generated all three software-copyright DOCX/PDF artifacts after the template and screenshot fixes. The current page counts are: user manual 53 pages, source-program submission 60 pages, and info sheet 2 pages, all rendered on A4.
- [x] (2026-04-24 15:55 CST) Reinstalled the local LibreOffice Homebrew cask because `/opt/homebrew/bin/soffice` pointed to a missing app bundle, then updated the generator to resolve the real LibreOffice executable before converting DOCX outputs to PDF.
- [x] (2026-04-23 20:30 CST) Added a repeatable soft-copyright document generator at `tools/soft_copyright/generate_second_bloom_soft_copyright_docs.py` that now builds the current Android-plus-backend software-copyright outputs directly from repo facts, real screenshot assets, and a fixed source-file order instead of relying on one-off manual editing.
- [x] (2026-04-23 20:30 CST) Generated the current software-copyright application artifacts under `output/doc`: `Second Bloom旧衣新生系统软件著作权用户手册.docx/.pdf`, `Second Bloom旧衣新生系统软件著作权源程序-普通交存.docx/.pdf`, and `新软件信息采集表-Second Bloom版.docx/.pdf`.
- [x] (2026-04-23 20:30 CST) Verified the generated page counts against the intended filing shape: the user manual currently renders to 53 pages, the source-program submission currently renders to 60 pages with front-and-back 30-page extraction, and the info sheet currently renders to 3 pages.
- [x] (2026-04-23 20:30 CST) Collected and normalized the current screenshot asset set for the handbook under `output/doc/assets/soft-copyright`, mixing fresh emulator captures for the live remodel flow (`01` through `07`) with previously captured real Second Bloom screen assets for wardrobe, planet, and profile/login states (`08` through `11`).
...
- [x] (2026-04-26 15:34 CST) Stabilized `sh gradlew connectedDebugAndroidTest` on the current emulator lane. The command now runs 18 Compose instrumentation tests on `emulator-5554` and completes successfully after the preview editor test switched from direct text scrolling to stable test-tag scrolling.
...
- Observation: the emulator can have TCP connectivity while HTTPS requests still fail because its DNS path resolves public domains incorrectly unless it uses the host proxy.
  Evidence: before setting the emulator proxy, `dig +short second-bloom-backend-juliantiens-projects.vercel.app` returned `202.160.128.16` and an IPv6 address under `2a03:2880:...`, while `dig +short www.google.com` returned `31.13.92.37`; emulator `NetworkMonitor` repeatedly logged `SSLHandshakeException: connection closed` and `PARTIAL_CONNECTIVITY`. After `settings put global http_proxy 10.0.2.2:7897`, emulator Wi-Fi became `VALIDATED`, Vercel logs recorded `POST /analyze-garment` at 18:02 CST, and the UI rendered the successful analysis summary for `55.jpg`.

- Observation: the hosted backend was healthy inside Vercel, but the Android-configured production alias was not a reliable public client endpoint from this network.
  Evidence: Vercel reported latest production deployment `dpl_kFo5fPZoENbrJgYHG8ooL6Fp6VBT` as `READY`, and Vercel-authenticated fetches to `https://second-bloom-backend.vercel.app/health/live` and `/health/ready` returned `200 {"status":"ok"}`. Ordinary `curl` to the same alias failed during TLS with `SSL_ERROR_SYSCALL`, while DNS resolved `second-bloom-backend.vercel.app` to `118.184.26.113`. After disabling Vercel SSO protection, the stable team alias `https://second-bloom-backend-juliantiens-projects.vercel.app` returned `200` for both health probes through ordinary `curl`.

- Observation: the current emulator's local app data did not include saved analysis or plan records, so natural runtime navigation could not reach the plan, true-image editor, or final-result pages during the visual smoke pass.
  Evidence: the profile screen UI tree reported `识别 0 条 · 方案 0 条`; those flow pages were still covered by source-level inset application and successful debug/androidTest compilation, while reachable runtime pages were checked with adb screenshots and UI-tree bounds.

- Observation: Clerk login itself now succeeds locally, but the currently configured hosted backend alias does not expose the authenticated history bootstrap route used by Android.
  Evidence: after sign-in, logcat showed an uncaught `IOException: The page could not be found` from `HttpHistorySyncApi.bootstrapHistory()` via `/me/history/bootstrap`, and a direct `curl -i https://second-bloom-backend.vercel.app/me/history/bootstrap` returns Vercel `404 NOT_FOUND` instead of an authenticated API response.

- Observation: the earlier unit-test source resolution failure was caused by a corrupted Gradle/Kotlin build cache state, not by missing production code.
  Evidence: `sh gradlew testDebugUnitTest --console=plain` initially failed with many unresolved references after a Kotlin daemon cache failure; `sh gradlew clean testDebugUnitTest --console=plain` rebuilt the classpath and passed, and subsequent `sh gradlew testDebugUnitTest --console=plain` runs now pass without `clean`.

- Observation: the current Android instrumentation blocker is resolved on the available emulator lane.
  Evidence: `adb devices -l` reports `emulator-5554`, and `sh gradlew connectedDebugAndroidTest --console=plain` now runs 18 tests on `Pixel_9_Pro_XL(AVD) - 16` with `BUILD SUCCESSFUL` after the preview editor test uses stable test tags for lazy-list scrolling.

- Observation: the Android lint blocker was confined to test composition code and is now fixed.
  Evidence: `sh gradlew lintDebug --console=plain` had failed with 3 `StateFlowValueCalledInComposition` errors in `RemodelScreensTest.kt`; after replacing in-composition `viewModel.uiState.value` reads with `collectAsState()`, `sh gradlew lintDebug --console=plain` completes successfully.

- Observation: full backend integration verification is currently an environment blocker, not a backend code blocker.
  Evidence: `.venv/bin/python -m compileall -q app api workers`, `.venv/bin/python -m pytest -q -rs`, and `.venv/bin/python -m pip wheel . --no-deps -w /tmp/second-bloom-backend-wheel` pass in `/Users/peng/AndroidStudioProjects/second-bloom-backend`, but `docker compose up -d postgres redis minio minio-init` cannot run because `docker`, `colima`, and `podman` are unavailable on `PATH`.

- Observation: the Android app was already sending `responseLanguage = "zh"` when the UI language was Chinese, but the sibling backend dropped that value before calling its plan provider.
  Evidence: `DefaultRemodelRepository.generatePlans(...)` passes `responseLanguage.wireValue`, while `/Users/peng/AndroidStudioProjects/second-bloom-backend/app/services/plan_service.py` rebuilt `GenerateRemodelPlansInput` without `response_language`, causing the dataclass default `en` to reach the Qwen/real-provider layer.
...
- Observation: an earlier Android 16 emulator run did not provide a usable path for the Compose instrumentation suite because the test stack failed inside Espresso before any app-level assertion could run.
...
- Decision: Treat the emulator host proxy setting as required local test setup for hosted-backend smoke tests on this machine.
  Rationale: the production backend works through the host proxy and the emulator can reach that proxy at `10.0.2.2:7897`, while the emulator's unproxied DNS/HTTPS path resolves key domains to incorrect IPs and fails before requests reach Vercel. This keeps Android code unchanged and isolates the fix to machine-specific network setup.
  Date/Author: 2026-05-08 / Codex

- Decision: Use the reachable Vercel team alias for local Android debug builds until a custom API domain is bound.
  Rationale: the app needs an ordinary HTTPS endpoint that an emulator or handset can call without Vercel login cookies or DNS overrides. The original `second-bloom-backend.vercel.app` alias still fails TLS in the current network path, while `second-bloom-backend-juliantiens-projects.vercel.app` reaches the same production deployment and now answers health probes publicly after SSO deployment protection was disabled.
  Date/Author: 2026-05-08 / Codex
...
Revision note (2026-04-26 15:29 CST, Codex): updated the plan after fixing the inspiration feed's status-bar overlap by adding root status-bar padding to `InspirationScreen`, verifying `assembleDebug` and `installDebug` successfully, and capturing emulator evidence that the `灵感空间` title now starts below the status bar.

Revision note (2026-05-08 17:49 CST, Codex): updated the plan after implementing shared Compose inset helpers across top-level and flow screens, moving the custom bottom navigation above the system gesture area, and verifying the debug build, unit tests, androidTest APK compilation, and adb smoke evidence for all currently reachable major pages.

Revision note (2026-05-10 16:27 CST, Codex): updated the plan after tracing the missing final-effect image to Android emulator access failures for `*.public.blob.vercel-storage.com` despite completed preview jobs and valid generated assets. The fix now routes persistent preview assets through the hosted backend `/preview-assets/{assetId}` endpoint, adds Android-side Blob URL fallback rewriting for saved history records, switches final-preview image rendering to an `HttpURLConnection`-based loader consistent with the real API client path, updates the API note to document app-served preview asset URLs, and verifies the Android URL regression/unit coverage plus backend service coverage.
