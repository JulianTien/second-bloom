...
- [ ] Stabilize `sh gradlew connectedDebugAndroidTest` on the current emulator lane. The command now runs on `emulator-5554` and executes product assertions, but the suite still fails because `previewEditorScreen_showsMicroAdjustmentControlsAndGenerateButton` cannot find the expected `额外说明` node during `performScrollTo()`.
...
- Observation: the current Android instrumentation blocker has moved from framework-level Espresso incompatibility to a specific UI-test expectation mismatch.
  Evidence: `adb devices -l` reports `emulator-5554`, and `sh gradlew connectedDebugAndroidTest --console=plain` ran 18 tests on `Pixel_9_Pro_XL(AVD) - 16`; only `previewEditorScreen_showsMicroAdjustmentControlsAndGenerateButton` failed because `performScrollTo()` could not find a node whose text contains `额外说明`, even though `PreviewEditorScreen.kt` still defines the card title at line 207.

- Observation: the current Android lint blocker is confined to test composition code, not production source.
  Evidence: `sh gradlew lintDebug --console=plain` failed with 3 `StateFlowValueCalledInComposition` errors at `app/src/androidTest/java/com/scf/secondbloom/RemodelScreensTest.kt` lines 762, 765, and 768, where the test reads `viewModel.uiState.value` inside `setContent`.
